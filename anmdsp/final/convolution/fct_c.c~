...
{ 
	...
	for(count=0 ; count < 9 ; count++)
		sum += pix_buf[count]*coeff_buf[count];
	return(sum);	       
}
...
{   
	int val;
	char coeff_buf[]={-1,-1,-1,-1,-1,8,-1,-1,-1,-1};
	unsigned char pix_buf[9];
	int k = 0;
	printf("Debut");
...
if(((k+1)%width==0)&&(k!=width-1)&&(k!=height*width-1)){
      ...
      pix_buf[8]=0;
				 ...
				 val = fct_t(pix_buf,coeff_buf)/8;
				if (val< 0)
					val = 0;
				if (val > 255)
					val = 255;
				tab_image_source[k]=val;
                                //tab_image_final[k]=val;

		if (color_max<val)
		{
		       	color_max = (unsigned char)val;
